...
#+DESCRIPTION: Personal notes for installing ArchLinux on a workstation, booting a single LUKS-encrypted system partition holding LVM volumes through GRUB.
...
We will boot bare-metal installations over UEFI because we are modern people. I
am perfectly fine with having *sda1* un-encrypted and the whole system data on
*sda2* encrypted with a single passphrase. The use of LVM gives the
...
disk encryption and basic networking capabilities. From here on we should check
the [[https://wiki.archlinux.org/title/General_recommendations][General recommendations]] to complete the setup of users, services, system
