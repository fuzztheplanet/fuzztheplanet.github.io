...
#+DESCRIPTION: How to keep a git configuration, an SSH key and a GPG signing key on a USB stick, so none of it has to persist on the machine you are working from.
#+DATE: [2025-07-07 Mon]
...
material onto. By no means I recommend plugin such a USB stick into non-trusted
devices.
...
These environment variables are set and unset using shell functions defined in
//mnt/usb/alias.sh/:
