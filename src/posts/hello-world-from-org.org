...
#+DESCRIPTION: Yet another simple publishing setup for a static website built with Emacs's Org mode, documented end to end for self-reference.
#+DATE: [2025-05-25 Sun]
...
#+begin_src sh
skywhi@dreamland:~/blog$ echo "Hello World Wild Web!" > src/posts/hello.org
...
*www/* contains the complete exported website.
...
and keeps only the pieces that are actually served.
...
mathjax:
    # Only download and serve the MathJax components needed.
    ...
    curl -fsSL $(NPM)/@mathjax/$(MATHJAX_FONT)-font/-/$(MATHJAX_FONT)-font-$(MATHJAX_VERSION).tgz \
